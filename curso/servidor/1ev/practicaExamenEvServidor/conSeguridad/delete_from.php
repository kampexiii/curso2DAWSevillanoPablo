...
if (!$error && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($modo === 'auto') {
        // CAMBIA esta condicion DELETE a la que pida el examen
        $sql = "DELETE FROM $table WHERE nota < 8 OR activo = 0";
        $stmt = $conexion->prepare($sql);
        $stmt->execute();
        $eliminados = $stmt->rowCount();
        $mensaje = "Eliminacion automatica OK. Registros borrados: $eliminados";
        $detalle = "Regla usada: nota < 8 o inactivo. Cambia la condicion al enunciado.";
    } elseif ($modo === 'manual') {
        $id = (int)($_POST['id'] ?? 0);
        if ($id <= 0) {
    ...
    <?php if (!$error): ?>
        <div class="container">
            ...
            <form method="post">
                ...
                <p>Cambia la sentencia DELETE en el PHP segun el enunciado.</p>
            </form>
        </div>
